 ...
 */
export function serverClientCodeSplit(watchMode = false) {
    return {
        ...
        setup(build) {
            ...
            if (process.env.DEVVIT_BUNDLE_STRIP_PLUGINS === '1') {
                ...
                build.onResolve({ filter: publicApiFilter }, (args) => {
            ...
            // of a ReferenceError.
            build.onLoad({ filter: /.*/, namespace: SERVER_ONLY_NAMESPACE }, async (args) => {
 ...
 * @param code
 */
export function postProcessServerStubCode(code) {
    return Array.from(code.matchAll(/var require_([a-zA-Z_-]+) = __commonJS\(\{\n[^\n]*"server-only:/gm)).reduce((code, [_, stub]) => code.replaceAll(`__toESM(require_${stub}(), 1)`, `require_${stub}()`), code);
}
