...
export async function writeVariableToDotEnv(envVariable, value, envPath) {
    let rawEnvContent = '';
...
export async function deleteVariableFromDotEnv(envVariable, envPath) {
    let rawEnvContent = '';
    if (await isFile(envPath)) {
        rawEnvContent = await readFile(envPath, 'utf-8');
    }
...
export function upsertToRawEnvContent(envVariable, value, rawEnvContent) {
    const envContent = {
        ...parseEnv(rawEnvContent),
        [envVariable]: value, // Overwrite or add the variable
    };
    return (Object.entries(envContent)
        .map(([k, v]) => `${k}=${v}`)
        .join(EOL) + EOL);
}
export function deleteFromRawEnvContent(envVariable, rawEnvContent) {
