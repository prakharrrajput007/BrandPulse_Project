...
import { getPaymentsConfig, makePaymentsConfig, readProducts, validateProductIcon, } from './paymentsConfig.js';
...
describe('Read product.json', () => {
    ...
    it('does not return any products if the files does not exist', async () => {
        ...
        const products = await readProducts({ root: PROJECT_ROOT });
    ...
    it('throws an error if products.json is not formatted properly', async () => {
        ...
        await expect(() => readProducts({ root: PROJECT_ROOT })).rejects.toThrowError('products.json validation error');
    });
});
describe('Get payments config', () => {
    it('throws an error if the bundle does not provide a PaymentProcessor', () => {
        const bundle = { assetIds: {}, code: '', webviewAssetIds: {} };
        expect(() => getPaymentsConfig(undefined, bundle, MOCK_PRODUCTS_JSON.products)).toThrowError(/You have a `products\.json` with products, but your app does not handle payment processing of those products./);
    });
    it('throws an error if the product images are not included in the assets', () => {
        ...
        const bundle = { ...MOCK_BUNDLE, assetIds: { 'exists.jpg': 'abc123' } };
        expect(() => getPaymentsConfig(undefined, bundle, products)).toThrowError(`Product images ${productImage} are not included in the assets`);
    });
    it('throws an error if no products are found ', () => {
        expect(() => getPaymentsConfig(undefined, MOCK_BUNDLE, [])).toThrowError('you must specify products in the `src/products.json` config file');
    });
    it('throws error if a product has metadata starting with "devvit-"', () => {
        ...
        expect(() => getPaymentsConfig(undefined, MOCK_BUNDLE, [invalidProduct])).toThrowError('Products metadata cannot start with "devvit-". Invalid keys: devvit-invalid');
    });
    it('creates payments config products.json is found and formatted properly', () => {
        expect(getPaymentsConfig(undefined, MOCK_BUNDLE, MOCK_PRODUCTS_JSON.products)).toStrictEqual(makePaymentsConfig(MOCK_PRODUCTS_JSON.products));
    });
    ...
    it('ignores product image asset verification if option is set to false', () => {
        ...
        expect(getPaymentsConfig(undefined, bundle, products, false)).toStrictEqual(makePaymentsConfig(products));
    });
});
describe('Validate product icon', () => {
    const imgPath = (filename) => path.join(__dirname, 'test-images', filename);
    it('rejects non-images', () => {
        expect(() => validateProductIcon(imgPath('not-an-image.png'))).toThrowError('not a valid image');
    });
    it('rejects non pngs', () => {
    ...
    it('rejects small images', () => {
        expect(() => validateProductIcon(imgPath('too-small.png'))).toThrowError('must be at least 256x256');
    });
