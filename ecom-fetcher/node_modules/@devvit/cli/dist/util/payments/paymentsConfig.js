...
import { Currency, Environment, PaymentProcessorDefinition, PaymentsServiceDefinition, } from '@devvit/protos/payments.js';
 ...
 */
export async function readProducts(project) {
 ...
 */
export function getPaymentsConfig(mediaDir, bundle, products, verifyProductImgAssets = true) {
    checkProductsConfig(mediaDir, products, bundle, verifyProductImgAssets);
    return makePaymentsConfig(products);
}
function checkProductsConfig(mediaDir, products, bundle, verifyProductImgAssets = true) {
    const hasProducts = products.length > 0;
    const usesPaymentPlugin = !!bundle.dependencies?.uses.find((uses) => uses.typeName === PaymentsServiceDefinition.fullName);
    const providesPaymentProcessor = !!bundle.dependencies?.provides.find((prv) => prv.definition?.fullName === PaymentProcessorDefinition.fullName);
    if (!hasProducts) {
        if (providesPaymentProcessor || usesPaymentPlugin) {
            throw new Error('To fully support the Payment capability in our app, you must specify products in the `src/products.json` config file. Please refer to https://developers.reddit.com/docs/capabilities/payments for more details.');
        }
    ...
    if (!providesPaymentProcessor) {
        throw new Error('You have a `products.json` with products, but your app does not handle payment processing of those products. Please refer to https://developers.reddit.com/docs/capabilities/payments for documentation to enable the payments feature.');
    }
    for (const product of products) {
        if (product.metadata) {
            ...
            if (invalidKeys.length > 0) {
                throw new Error(`Products metadata cannot start with "devvit-". Invalid keys: ${invalidKeys.join(', ')}`);
            }
        }
    }
    const missingAssets = [];
    if (verifyProductImgAssets) {
        ...
        for (const product of products) {
            ...
            const missing = Object.values(product.images).filter((assetPath) => !assets.includes(assetPath));
        ...
        if (missingAssets.length > 0) {
            throw new Error(`Product images ${missingAssets.join(', ')} are not included in the assets of the bundle. Please ensure that the image is included in the /assets directory.`);
        }
    }
}
function parseProductsFileJSON(fileContents) {
    try {
        ...
        if (err instanceof Error) {
            ...
            const newLine = '\n\t- ';
            throw new Error(`${PRODUCTS_JSON_FILE} ${err.message}:${newLine}${err.cause.map((e) => e.toString().replace(/^instance\./i, '')).join(newLine)}`);
        }
        throw err;
    }
}
export function makePaymentsConfig(products) {
...
export function validateProductIcon(assetPath) {
    ...
    if (width !== height) {
        throw new Error(`Product icon ${assetPath} must be square. The provided asset is ${width}x${height}px`);
    }
    if (!width || width < 256) {
        throw new Error(`Product icon ${assetPath} must be at least 256x256. The provided asset is ${width}x${height}px`);
    }
}
