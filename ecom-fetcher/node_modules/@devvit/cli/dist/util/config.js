import { DOT_DEVVIT_DIR_FILENAME, MY_PORTAL_ENABLED, REDDIT_DOT_COM, STAGE_USER_NAME, } from '../lib/config.js';
/** Relative build products directory filename. Used for Bundle JSON. */
export const distDirFilename = 'dist';
export const REDDIT_OAUTH_API = (() => {
...
export const DEVVIT_GATEWAY_URL = (() => {
    if (MY_PORTAL_ENABLED) {
        return `https://devvit-gateway.${STAGE_USER_NAME}.snoo.dev`;
    }
    return 'https://devvit-gateway.reddit.com';
})();
export const DEVVIT_DISABLE_EXTERN_DEVVIT_PROTOS = process.env.DEVVIT_DISABLE_EXTERN_DEVVIT_PROTOS === '1';
export const GQL_QUERY_URL = (() => {
    if (MY_PORTAL_ENABLED) {
...
if (MY_PORTAL_ENABLED) {
    console.debug('Using a staging environment. To do so, toggling node TLS protections off. Please note this will result in node warnings.');
