...
import { isDependencyManagedByDevvit, isDevvitDependency, } from '@devvit/shared-types/isDevvitDependency.js';
...
class UpdateApp extends DevvitCommand {
    async init() {
        await super.init();
...
export async function matchDevvitPackageVersions(projectRoot, devvitVersion, forceUpdate = false) {
    ...
    console.log(chalk.bold(`Found dependencies that need to be synced to the local devvit version (${devvitVersion.version}): `));
    tree.display();
...
function overwriteDependencies(devvitVersion, forceUpdate, dependencies) {
    const dependenciesWeManage = Object.entries(dependencies).filter(([depName]) => isDependencyManagedByDevvit(depName));
...
function getTargetSemVer(devvitVersion, dep) {
    ...
    const parsedSemVer = semver.parse(cliPackageJSON.dependencies[dep] ?? cliPackageJSON.devDependencies[dep]);
    if (!parsedSemVer) {
        throw new Error("Couldn't find the CLI's dependency for " +
            dep +
