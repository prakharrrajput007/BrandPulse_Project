...
var _PlaytestServer_instances, _PlaytestServer_connections, _PlaytestServer_logConfig, _PlaytestServer_logger, _PlaytestServer_server, _PlaytestServer_timer, _PlaytestServer_checkLiveness, _PlaytestServer_onServerErr, _PlaytestServer_onSockClosed, _PlaytestServer_onSockErr, _PlaytestServer_onSockMsg, _PlaytestServer_onSockOpened, _PlaytestServer_onVerifyClient;
...
export class PlaytestServer {
    constructor(logConfig, logger) {
        ...
        _PlaytestServer_onServerErr.set(this, (ev) => {
            __classPrivateFieldGet(this, _PlaytestServer_logger, "f").warn(`connection server error ${ev.error}: ${ev.message}`);
        });
        _PlaytestServer_onSockErr.set(this, (ev) => {
            __classPrivateFieldGet(this, _PlaytestServer_logger, "f").warn(`socket error ${ev.error}: ${ev.message}`);
        });
        _PlaytestServer_onSockOpened.set(this, (sock) => {
            ...
            sock.onmessage = (ev) => __classPrivateFieldGet(this, _PlaytestServer_instances, "m", _PlaytestServer_onSockMsg).call(this, con, ev);
            sock.onerror = __classPrivateFieldGet(this, _PlaytestServer_onSockErr, "f");
            sock.on('pong', () => {
                con.active = Date.now();
            });
        });
         ...
         */
        _PlaytestServer_onVerifyClient.set(this, (info) => {
            // Get just the domain portion of the Origin header, ignoring protocol and
            // port.
            const domain = new URL(info.origin).hostname;
            const allowed = ['localhost', '127.0.0.1', '0.0.0.0', '[::1]', 'reddit.com', 'snoo.dev'].some((allowed) => allowed === domain || domain.endsWith(`.${allowed}`));
            if (!allowed)
                __classPrivateFieldGet(this, _PlaytestServer_logger, "f").warn(`ignoring unknown connection from ${info.origin}`);
            return allowed;
        });
        __classPrivateFieldSet(this, _PlaytestServer_logConfig, logConfig, "f");
    ...
    open() {
        ...
        __classPrivateFieldSet(this, _PlaytestServer_server, new WebSocketServer({ port, verifyClient: __classPrivateFieldGet(this, _PlaytestServer_onVerifyClient, "f") }), "f");
        ...
        __classPrivateFieldGet(this, _PlaytestServer_server, "f").on('error', __classPrivateFieldGet(this, _PlaytestServer_onServerErr, "f"));
    }
...
_PlaytestServer_connections = new WeakMap(), _PlaytestServer_logConfig = new WeakMap(), _PlaytestServer_logger = new WeakMap(), _PlaytestServer_server = new WeakMap(), _PlaytestServer_timer = new WeakMap(), _PlaytestServer_checkLiveness = new WeakMap(), _PlaytestServer_onServerErr = new WeakMap(), _PlaytestServer_onSockErr = new WeakMap(), _PlaytestServer_onSockOpened = new WeakMap(), _PlaytestServer_onVerifyClient = new WeakMap(), _PlaytestServer_instances = new WeakSet(), _PlaytestServer_onSockClosed = function _PlaytestServer_onSockClosed(con) {
    if (con.url)
...
}, _PlaytestServer_onSockMsg = function _PlaytestServer_onSockMsg(con, ev) {
    ...
    if (msg.callState)
        for (const log of msg.callState.logs)
            __classPrivateFieldGet(this, _PlaytestServer_logger, "f").log(formatAppLogMessage(log, __classPrivateFieldGet(this, _PlaytestServer_logConfig, "f"), true));
    if (msg.connect) {
